training = loadMNIST('Training set images.idx3-ubyte','Training set label.idx1-ubyte');
test = loadMNIST('Test set 10k images.idx3-ubyte','Test set label.idx1-ubyte');
...
attivazioneInterna=elu();
identita=identita();
funErr=crossEntropy();
if(tipo==0)
    net = trainingRProp([784 50 10],{ attivazioneInterna identita}, pesi, funErr, trainingSet, validationSet, nEpoche, eta,etaP, etaN, variation,soglia);
elseif(tipo==1)
